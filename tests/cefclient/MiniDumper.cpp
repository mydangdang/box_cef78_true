...
{
	//PostMessage( CIEProgressWndSink::GetInstance()->GetParentHWnd(),
	//	WM_PAGE_EXCEPTION, (WPARAM)CIEProgressWndSink::GetInstance()->GetCurHWnd(), 0 );
	if( !s_pMiniDumper )
...
void CMiniDumper::setMiniDumpFileName( void )
{
    ...
    _tcssprintf( m_szMiniDumpPath,
                 _T( "%s\\%s.%ld.dmp" ),
        UtilString::SA2W(client::DataPath()).c_str(),
                 m_szAppBaseName,
                 currentTime );
}
