...
scoped_refptr<client::RootWindow> RootWindowManager::CreateRootWindowAsTool(
    bool bOSR,
    bool bTopMost,
    bool bToolWnd, 
    ...
    CefRefPtr<CefRequestContext> ctx)
{
	...
	root_window->InitAsTool(this, bOSR, bTopMost,bToolWnd,url,sTag,rcPos,nAction,ctx);
...
bool RootWindowManager::HasRootWindowAsTool(const std::string& sTag)
{
	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it) {
		RootWindow* root_window = (*it);
        if (root_window->IsToolWnd() && sTag == root_window->GetMainBrowserTag())
        {
            return true;
        }
	}
...
scoped_refptr<RootWindow> RootWindowManager::GetWindowForBrowser(
  ...
  RootWindowSet::const_iterator it = root_windows_.begin();
  for (; it != root_windows_.end(); ++it) {
      if((*it)->OwnerBrowser(browser_id))
          return *it;
  }
  return NULL;
}

scoped_refptr<client::RootWindow> RootWindowManager::GetWindowForBrowser(const std::string& szTag) const
{
	REQUIRE_MAIN_THREAD();

	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it) {
		if ((*it)->OwnerBrowser(szTag))
			return *it;
	}
	return NULL;
}
...
void RootWindowManager::BroadCastMsg(int nBrowserID, const std::string& scallerTag, CefRefPtr<CefDictionaryValue> msg, int nTargetType)
{
	if (!CURRENTLY_ON_MAIN_THREAD()) {
		// Execute this method on the main thread.
		MAIN_POST_CLOSURE(base::Bind(&RootWindowManager::BroadCastMsg,base::Unretained(this), nBrowserID, scallerTag,msg,nTargetType));
		return;
	}
    ...
    if(!nTargetType)
	{
        ...
        for (; it != root_windows_.end(); ++it)
        {
            CefRefPtr<CefDictionaryValue> duplicateMsg = msg->Copy(false);
...
void RootWindowManager::PostCustomMsg(int nBrowserID, std::string& scallerTag,const std::string& sMsgName, CefRefPtr<CefDictionaryValue> msg)
{
	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it) {
		if ((*it)->OwnerBrowser(nBrowserID))
		{
            CefRefPtr<CefDictionaryValue> duplicateMsg = msg->Copy(false);
			(*it)->PostCustomMsg(nBrowserID, scallerTag, sMsgName, duplicateMsg);
			break;
		}
	}
}
...
CefRefPtr<CefRequestContext> RootWindowManager::GetRequestContext(
    RootWindow* root_window, bool bNew) {
  REQUIRE_MAIN_THREAD();

  if (request_context_per_browser_ || bNew) {
    // Create a new request context for each browser.
    CefRequestContextSettings settings;

	
	std::stringstream ss;
	ss << client::DataPath() << "\\cache\\tmp" << time(NULL);
...
void RootWindowManager::OnRootWindowDestroyed(RootWindow* root_window) {
  ...
  for (; it != root_windows_.end(); ++it)
  {
      if (!(*it)->IsToolWnd())
      {
          bStillHaveTopWnd = true;
      }
	  CefRefPtr<CefDictionaryValue> browserCreatedMsg = CefDictionaryValue::Create();
	  browserCreatedMsg->SetString("tag", szTagName);
	  browserCreatedMsg->SetInt("browserid", nBrowserID);
	  (*it)->PostCustomMsg(nBrowserID, szTagName, "onTopBrowserClosed", browserCreatedMsg);
  }
...
void RootWindowManager::OnBrowserCreated(RootWindow* root_window,
  ...
  if (root_window == active_root_window_) {
    ...
    for (; it != root_windows_.end(); ++it)
    {
        ...
        std::string szTagName = root_window->GetBrowserTagFromID(nBrowserID);
        CefRefPtr<CefDictionaryValue> browserCreatedMsg = CefDictionaryValue::Create();
        browserCreatedMsg->SetString("tag", szTagName);
        browserCreatedMsg->SetInt("browserid", nBrowserID);
        (*it)->PostCustomMsg(nBrowserID,szTagName,"onTopBrowserCreated", browserCreatedMsg);
    }
}
...
void RootWindowManager::OnTopFullHotKey()
{
	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it)
        (*it)->OnTopFullHotKey();
...
void RootWindowManager::OnBossHotKey()
{
	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it)
		(*it)->OnBossHotKey();
...
void RootWindowManager::ReleaseBossKey()
{
    RootWindowSet::const_iterator it = root_windows_.begin();
    if ((*it)->IsUnderBoss())
    {
...
void RootWindowManager::OnActiveApp(bool bActive, HWND hWnd)
{
	RootWindowSet::const_iterator it = root_windows_.begin();
	for (; it != root_windows_.end(); ++it)
        (*it)->OnAppActived(bActive,hWnd);
}
