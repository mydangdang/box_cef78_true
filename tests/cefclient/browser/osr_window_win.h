...
#endif
{
 public:
  ...
  LRESULT CALLBACK OsrWndProc(HWND hWnd,UINT message, WPARAM wParam,LPARAM lParam);
 private:
  ...
  void OnEditNodeFocused(int nX, int nY) OVERRIDE;
  ...
  bool UpdateDirtyRect(Gdiplus::Bitmap **pDstBtmp, const void* buffer, const RectList& dirtyRects,int nWidth,int nHeight);

  HBRUSH m_hEraser = NULL;
  ...
  Gdiplus::Bitmap* m_pOSRBitmap = NULL;
  Gdiplus::Bitmap* m_pPopBitmap = NULL;
