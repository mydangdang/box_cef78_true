...
//	frame->LoadURL(test_runner::GetDataURI(ss.str(), "text/html"));
	...
	std::string szURL = RTCTX::CurPath() + "\\staticres\\err.html?url="+failed_url;
	frame->LoadURL(szURL);
}
...
bool ClientHandler::OnProcessMessageReceived(
  ...
  if (message_name == kFocusedNodeChangedMessage) {
	...
	{
		int nNodeX = message->GetArgumentList()->GetInt(1);
		int nNodeY = message->GetArgumentList()->GetInt(2);
		OnEditNodeFocused(nNodeX, nNodeY);
	}
    return true;
  }
...
bool ClientHandler::OnPreKeyEvent(CefRefPtr<CefBrowser> browser,
  ...
  if (!event.focus_on_editable_field) {
      if (event.type == KEYEVENT_RAWKEYDOWN && event.windows_key_code == VK_F12) {
		  ...
		  if (!PathFileExistsA(szAppFile.c_str()))
			  return false;
#endif
          ShowDevTools(browser, CefPoint());
          return true;
...
bool ClientHandler::OnBeforePopup(
  ...
  CefRefPtr<client::RootWindow> rootWindow = MainContextImpl::Get()->GetRootWindowManager()->GetWindowForBrowser(browser->GetIdentifier());
  if (rootWindow && rootWindow->IsHookPopupLink(browser->GetIdentifier()))
  {
      NotifyBrowserNavigate(browser, target_frame_name, target_url);
      return true;
  }
  
  if (windowInfo.width <= 0 || windowInfo.width >= 4000 || windowInfo.height <= 0 || windowInfo.width >= 4000)
  {
	  RECT rcInvoker = { 0 };
	  HWND hInvoker = browser->GetHost()->GetWindowHandle();
	  GetWindowRect(hInvoker, &rcInvoker);

	  windowInfo.width = rcInvoker.right - rcInvoker.left;
	  windowInfo.height = rcInvoker.bottom - rcInvoker.top;
  }
  if (windowInfo.x == -1) {
      ...
      windowInfo.width *= fScal;
      windowInfo.height *= fScal;
	  windowInfo.x = (int)((GetSystemMetrics(SM_CXSCREEN) - windowInfo.width) * 1.0f / 2);
	  windowInfo.y = (int)((GetSystemMetrics(SM_CYSCREEN) - windowInfo.height) * 1.0f / 2);
  }
...
void ClientHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser) {
  ...
  char szOut[256] = { 0 };
  sprintf_s(szOut, 256, "Browser:[%d] OnAfterCreated\n", browser->GetIdentifier());
...
plugins.plugins_disabled_exceptions = []
  ...
  value->SetInt(1);
  browser->GetHost()->GetRequestContext()->SetPreference("profile.default_content_setting_values.plugins", value, error);

  CefRefPtr<CefValue> value2 = CefValue::Create();
  value2->SetBool(true);
  browser->GetHost()->GetRequestContext()->SetPreference("run_all_flash_in_allow_mode", value2, error);
...
void ClientHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser) {
  ...
  char szOut[256] = { 0 };
  sprintf_s(szOut, 256, "Browser:[%d] OnBeforeClose\n", browser->GetIdentifier());
  OutputDebugStringA(szOut);
...
{
    //禁用 onbeforeunload事件 默认准许;
    callback->Continue(true, "");
    return true;
}

void ClientHandler::OnEditNodeFocused(int nX, int nY)
{
...
void ClientHandler::ShowDevTools(CefRefPtr<CefBrowser> browser,
  ...
  if (!has_devtools) {
    ...
    // by ShowDevTools().
      RECT rcPos;
      GetWindowRect(host->GetWindowHandle(), &rcPos);
      CefPopupFeatures feature;
      feature.xSet = rcPos.left + 50;
      feature.ySet = rcPos.top + 50;
      feature.widthSet = rcPos.right - rcPos.left;
      feature.heightSet = rcPos.bottom - rcPos.top;
    has_devtools = CreatePopupWindow(browser, true, feature,
...
void ClientHandler::NotifyBrowserNavigate(CefRefPtr<CefBrowser> browser, std::string szTag, std::string szURL)
{
	if (!CURRENTLY_ON_MAIN_THREAD()) {
		// Execute this method on the main thread.
		MAIN_POST_CLOSURE(
			base::Bind(&ClientHandler::NotifyBrowserNavigate, this, browser, szTag,szURL));
		return;
	}

	if (delegate_)
		delegate_->OnBrowserNavigate(browser,szTag,szURL);
}
