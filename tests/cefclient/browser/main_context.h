...
class MainContext {
 public:
  ...
  virtual bool SysShutDownPlan(int nBrowserID,bool bAdd, int nLeftMinites, int nWarninigMinites) = 0;

  virtual void UpdateMemStorage(const std::string& sTag, const std::string& sValue) = 0;
  virtual std::string GetMemStorage(const std::string& sTag) = 0;
  virtual std::string GetVersion() = 0;
  virtual std::string GetBaseInfo() = 0;
  virtual std::wstring GetHardwarInfo() = 0;
  virtual void SetInfo(const std::string& szSID, const std::string& szCID, const std::string& szGID) = 0;
  virtual std::string GetInitFrameName() = 0;
  virtual RECT GetInitPos() = 0;
  virtual std::string GetClockName() = 0;
  virtual void OnClock(std::string szName) = 0;
  virtual void OnRecordScreenStop(std::string szTagName) = 0;
 protected:
  MainContext();
  virtual ~MainContext();
