...
// 	CreateRootWindow(CefBrowserSettings(), false);
    ...
    //���� �����ڵ� reqCtx 
    //�����빫�� cache �������ﴴ��һ���µ�rc auto reqCtx = delegate_->GetRequestContext(this);
...
{
//     ���ﴴ��һ�� tempwindow��Ϊparent
//     �Ƚ��ܵ�browsercreated�� createrootwindow
//     oncreate�����tabBrowserWindow showpopup �󶨹�ȥ;
//     ���������� createpopupbrowserһ��
//     ��ͬ����, window.open ��Ӧ�� onbeforepopup ���� ��ȡ��tempwindow, cefclient��  chrome�ڲ�ͨ�� host::creatbrowser�����괰��
//     Ȼ��onaftercreate  createrootwindow  Ȼ��oncreate��
//     ��������ģ�� window.open����Ҫ  tempwindow Ȼ�� hostcreatebrowser Ȼ�� onaftercreate createrootwindow��oncreate��
//     ע�� ����ҲҪ���� requectcontext
//     ����дһ��js����������
	// ����һ��js����   ���Ի�ͨ���� cache
...
void RootWindowWin::CreateRootWindow(const CefBrowserSettings& settings,
  ...
  DWORD dwStyle = 0;
  if (!isDevsTool) //ȥ�� WS_CAPTIONd �ж����� IE��Ƕ������
  {
...
LRESULT CALLBACK RootWindowWin::RootWndProc(HWND hWnd,
  ...
  case WM_ACTIVATEAPP:
  {
	  //���⴦��, ��Ϊ��popup���Ӵ���ģʽ ����activity�� popup���ߵ�bug
...
{
    //���������Ķ��ǩ���� ��ws_child ���л�������, ������popup���ws_child����, ���������ֻ��н�������
    //���������  ������ popup�Ӵ����� app deactive�� ���������ָ�  û���ָ�
    //��Ϊ activeapp�� ���и� active���Ӵ��ڳԵ���
    //�ֲ������Ӵ��ڷ�����, �����ᵼ�� ��ҳ����editʱ��bug
    //����ֱ�� ��activeapp�� foregroundҲ����, ��Ϊ ÿ�μ��� ��������ж��㴰�� �����յ� activeapp

    //����������һ�� ����    ��һ��activeapp ���� ����foreground����,  ��deactiveʱ���Ϊһ������
...
void UnSubclassWindow(HWND hWnd) {
  ...
  if (hParentWndProc) {
    ...
    if (hPreviousWndProc != reinterpret_cast<LONG_PTR>(SubclassedWindowProc)) {
      OutputDebugStringA("RestoreParentWndProc mismatch\n");
    }
  }
...
void RootWindowWin::NotifyDestroyedIfDone() {
  ...
  if (destroyed_notified_ || !window_destroyed_ || !browser_destroyed_)
    return;

  destroyed_notified_ = true;
