...
void BrowserWindowOsrWin::TabShow()
{
    Show();
}


void BrowserWindowOsrWin::TabHide()
{
    Hide();
}
...
LRESULT CALLBACK BrowserWindowOsrWin::OsrWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	REQUIRE_MAIN_THREAD();
	if (osr_window_) return osr_window_->OsrWndProc(hWnd, message, wParam, lParam);
