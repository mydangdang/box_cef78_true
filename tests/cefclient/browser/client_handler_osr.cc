...
void ClientHandlerOsr::OnEditNodeFocused(int nX, int nY)
{
    if (osr_delegate_) osr_delegate_->OnEditNodeFocused(nX, nY);
}

bool ClientHandlerOsr::OnTooltip(CefRefPtr<CefBrowser> browser, CefString& text)
{
    if (osr_delegate_) return osr_delegate_->OnTooltip(browser, text);
    return false;
}
