...
class RootWindow
 ...
 public:
  ...
  class Delegate {
   public:
    ...
    virtual CefRefPtr<CefRequestContext> GetRequestContext(
        RootWindow* root_window,bool bNew) = 0;
  ...
  virtual void InitAsTool(RootWindow::Delegate* delegate, 
      bool bOSR,
      bool bTopMost,
	  bool bToolWnd,
	  const std::string& url,
	  const std::string& sTag, RECT rcPos, int nAction,
	  CefRefPtr<CefRequestContext> ctx) = 0;
  ...
  virtual void OnTopFullHotKey();
  virtual void OnBossHotKey();
  virtual void OnAppActived(bool bActive,HWND hWnd);
  ...
  virtual void BroadCastMsg(int nCallerID, const std::string& sCallerTag, CefRefPtr<CefDictionaryValue> msgJson, int nTargetType);
  virtual void PostCustomMsg(int nCallerID, std::string& sTagName, const std::string& sMsgName, CefRefPtr<CefDictionaryValue> msgJson);

  virtual bool OwnerBrowser(int nBrowserID);
  virtual bool OwnerBrowser(const std::string& sTagName);

  virtual CefRefPtr<CefBrowser> GetChildBrowser(int nBrowserID);
  virtual CefRefPtr<CefBrowser> GetChildBrowser(const std::string& sTag);

  virtual std::string GetBrowserTagFromID(int nBrowserID);

  virtual int GetMainBrowserID();
  virtual std::string GetMainBrowserTag();

  virtual bool IsHookPopupLink(int nID);
  virtual void SetHookPopupLink(int nID,bool bHook);

  virtual bool IsToolWnd();
  virtual bool IsOSR();
  virtual bool IsUnderBoss();
 protected:
