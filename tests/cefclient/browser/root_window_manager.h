...
class RootWindowManager : public RootWindow::Delegate {
 public:
  ...
  scoped_refptr<RootWindow> CreateRootWindowAsTool(
      bool bOSR,
      bool bTopMost,
      bool bToolWnd,
  ...
  bool HasRootWindowAsTool(const std::string& sTag);
  ...
  scoped_refptr<RootWindow> GetWindowForBrowser(const std::string& szTag) const;
  ...
  void BroadCastMsg(int nBrowserID, const std::string& scallerTag, CefRefPtr<CefDictionaryValue> msg, int nTargetType);
  void PostCustomMsg(int nBrowserID, std::string& scallerTag, const std::string& sMsgName, CefRefPtr<CefDictionaryValue> msg);
  ...
  void OnBossHotKey();
  void ReleaseBossKey();
  void OnActiveApp(bool bActive, HWND hWnd);
 private:
  ...
  CefRefPtr<CefRequestContext> GetRequestContext(
      RootWindow* root_window,bool bNew) OVERRIDE;
